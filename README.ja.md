...
- **Branch（分岐）** — 会話を複製して別方向を探索；サイドバーでツリー表示・折りたたみ可能
- **Split View（画面分割）** — 2つのセッションを横/縦に並べて表示（区切り線をドラッグで調整）
- **Pin（お気に入り）** — 重要なセッションをサイドバー上部に固定（⭐）
- **自動タイトル** — 最初のメッセージからセッション名を自動生成
- **インライン名前変更 / 削除**
- **Stop（応答中断）** — 応答中にワンクリックで即座に中断
- **ストレージ整理** — 不要なClaude CLIセッションファイルを一括削除（🗑）
