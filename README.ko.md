...
- **Branch (분기)** — 현재 대화를 복제해 다른 방향 탐색; 사이드바에서 트리 구조로 접기/펼치기
- **Split View (화면 분할)** — 두 세션을 가로/세로로 나란히 표시 (구분선 드래그 조절)
- **Pin (즐겨찾기)** — 중요한 세션을 사이드바 상단에 고정 (⭐)
- **자동 제목** — 첫 메시지로 세션 이름 자동 생성
- **인라인 이름 변경 / 삭제**
- **Stop (응답 중단)** — 응답 중 클릭 한 번으로 즉시 중단
- **Storage 정리** — 불필요한 Claude CLI 세션 파일 일괄 삭제 (🗑)
